...
| 2026-06-06 | Do not submit `exp044` | Seed averaging reduced seed arbitrariness but CV 0.966383 was below `exp038` 0.966420 and `exp042` 0.966398 | Keep `exp038` as the best candidate; continue improving or calibrating before API submission |
| 2026-06-06 | Do not submit `exp047` yet | Fixed STAR 1.05 candidate matches best CV 0.966420, but API submission should first check submission history/limit and record confidence rationale | Treat `exp047` as the strongest self-model candidate for the next API submission review |
